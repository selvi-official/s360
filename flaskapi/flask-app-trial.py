...
def slaBreachedData():
    ...
    if bu == 'All':
        return json.dumps({'SLA Breached': getSlaResult(bu , project , from_date , to_date , 'Breached') ,
...
@app.route('/app/bu_based_sla_reliability')
def bubasedSla():
    bu = request.args.get("bu")
...
def sla_trends():
    ...
    from_date = request.args.get("from_date" , type=toDate)
    to_date = request.args.get("to_date" , type=toDate)
    return get_sla_trends(bu , product , from_date , to_date)
...
def okrCustomerOutagesv1():
    ...
    product = request.args.get('product')
    quarter = request.args.get('quarter')
    return getOkrCustomerOutagesv1(bu , product , quarter)
...
def okrOutages():
    ...
    product = request.args.get('product')
    quarter = request.args.get('quarter')
    return getOkrOutages(bu , product , quarter)
